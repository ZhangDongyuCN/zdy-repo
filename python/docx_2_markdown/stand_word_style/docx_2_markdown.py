...
def write_paragraph(doc, block, f_md, md_save_path, config, hyperlink_dict):
    try:
        ...
        # 处理段落内容
        # + 获取段落样式
        # + block.style.name的取值为['Heading 1', 'Heading 2', 'Heading 3', ... ,'Normal', 'List Paragraph']
        text_type = block.style.name

        # + 按照一定格式处理段落内容
        text = ''
        if text_type.find('Heading') != -1:  # 如果是标题
            # 拼接文本
            ...
            # 拼接文本
            for run in block.runs:
                if run.font.bold or run.font.cs_bold:  # 加粗
                    text += '**' + run.text + '**'
                elif run.font.italic or run.font.cs_italic:  # 斜体认定为行代码
                    text += '`' + run.text + '`'
                else:  # 普通文本
                    text += run.text
        ...
        print('[ERROR]', e)
        print('[ERROR] error in write_paragraph(), block is: {}'.format(block))
...
def write_table(block, f_md):
    try:
        ...
        print('[ERROR]', e)
        print('[ERROR] error in write_table(), block is: {}'.format(block))
